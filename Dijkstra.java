...
public class Dijkstra
{
    ...
    {
        //for each vertex in the map, go through its list of adjacent vertices and
        //compute the distance between the two - this is the cost of the edge

        //iterate through each vertex
        java.util.Iterator<Map.Entry<String, Vertex>> itr = vertexNames.entrySet().iterator();
        while(itr.hasNext())
        {
            //for each vertex, iterate through its adjacency list
            java.util.ListIterator<Edge> adjacentEdges = itr.next().getValue().adjacentEdges.listIterator();
            while(adjacentEdges.hasNext())
            {
                //get the source vertex and target vertex
                Edge adjacentEdge = adjacentEdges.next();
                Vertex source = adjacentEdge.source;
                Vertex target = adjacentEdge.target;

                //compute the distance between the source and target
                double distance = computeEuclideanDistance(source.x, source.y, target.x, target.y);

                //replace old edge with new edge of the updated distance
                adjacentEdges.set(new Edge(source, target, distance));
            }
        }
    }
    ...
    {
        //create a min heap
        BinaryHeap<Vertex> minHeap = new BinaryHeap<>();

        //Iterate through vertexNames map to set the distance and known variables for
        //each vertex
        java.util.Iterator<Map.Entry<String, Vertex>> vItr = vertexNames.entrySet().iterator();
        while(vItr.hasNext())
        {
            Vertex v = vItr.next().getValue();
            if(v.name.equals(s))
            {
                //set distance to 0 for starting vertex
                v.distance = 0;
            }
            else
            {
            ...
            //add to min heap
            minHeap.insert(v, v.distance);
        }

        //set the distance of vertex s to 0
        vertexNames.get(s).distance = 0;

        //set start's prev to null
        vertexNames.get(s).prev = null;

        while(!minHeap.isEmpty())
        {
            //get shortest unknown vertex
            Vertex v = minHeap.deleteMin();
            v.known = true;
            ...
            for(Edge edge : v.adjacentEdges)
            {
                ...
                //if adjacent vertex is not known, we can process it - it
                //hasn't been visited yet
                if(!adjacentVertex.known)
                {
                    //cost from v to adjacent vertex
                    double cost = edge.distance;

                    if(v.distance + cost < adjacentVertex.distance)
                    {
                        //update distance for adjacent vertex
                        adjacentVertex.distance = v.distance + cost;

                        //decrease the adjacent vertex's key in the min heap
                        minHeap.decreaseKey(adjacentVertex, v.distance + cost);

                        //set adjacent vertex's previous to v
                        adjacentVertex.prev = v;
                    }
                }
            }
        }
    ...
    {
        //do dijkstra's algorithm starting at vertex s
        doDijkstra(s);


        //list of edges from s to t
        List<Edge> path = new java.util.LinkedList<>();

        //start at vertex t (endpoint)
        ...
        while(v.prev != null)
        {
            ...
            //go through v's prev's list of adjacent vertices to find the edge from prev to v
            for(Edge edge : prev.adjacentEdges)
            {
                if(edge.target.equals(v))
                {
                    //add edge from prev to v to the list - add at front, so result is in order
                    path.add(0,edge);
                    break;
                }
            }
